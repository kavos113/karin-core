...
TEST_F(Transform2DTest, RowMajorData)
{
    Transform2D t;
    ...
    for (int i = 0; i < 16; ++i)
    {
        EXPECT_FLOAT_EQ((&expectedCol[0][0])[i], colData[i]);
    }
    ...
    for (int i = 0; i < 16; ++i)
    {
        EXPECT_FLOAT_EQ((&expectedRow[0][0])[i], rowData[i]);
    }
}
